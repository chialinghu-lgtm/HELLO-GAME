 ...
 * Customized for offline classroom games (No external API needed)
 */
(function(window){
...
QRRSBlock.RS_BLOCK_TABLE=[[1,26,19],[1,26,16],[1,26,13],[1,26,9],[1,44,34],[1,44,28],[1,44,22],[1,44,16],[1,70,55],[1,70,44],[2,35,17],[2,35,13],[1,100,80],[2,50,32],[2,50,24],[4,25,9],[1,134,108],[2,67,43],[2,33,15,2,34,16],[2,33,11,2,34,12],[2,86,68],[4,43,27],[4,43,19],[4,43,15],[2,98,78],[4,49,31],[2,32,14,4,33,15],[4,39,13,1,40,14],[2,121,97],[2,60,38,2,61,39],[4,40,18,2,41,19],[4,40,14,2,41,15],[2,146,116],[3,58,36,2,59,37],[4,36,16,4,37,17],[4,36,12,4,37,13],[2,86,68,2,87,69],[4,69,43,1,70,44],[6,43,19,2,44,20],[6,43,15,2,44,16],[4,101,81],[1,80,50,4,81,51],[4,50,22,4,51,23],[3,36,12,8,37,13],[2,116,92,2,117,93],[6,58,36,2,59,37],[4,46,20,6,47,21],[7,42,14,4,43,15],[4,133,107],[8,59,37,1,60,38],[8,44,20,4,45,21],[12,33,11,4,34,12],[3,145,115,1,146,116],[4,64,40,5,65,41],[11,36,16,5,37,17],[11,36,12,5,37,13],[5,109,87,1,110,88],[5,65,41,5,66,42],[5,54,24,7,55,25],[11,36,12,7,37,13]];
...
var QRCode = function(el, options) {
    ...
    this.correctLevel = options.correctLevel || QRErrorCorrectLevel.M;
    if (options.text) this.makeCode(options.text);
};

QRCode.prototype.makeCode = function(text) {
    var qr = new QRCodeModel(-1, this.correctLevel);
    qr.addData(unescape(encodeURIComponent(text)));
    qr.make();
    
    var count = qr.getModuleCount();
    var cellWidth = this.width / count;
    var cellHeight = this.height / count;
    ...
    for (var row = 0; row < count; row++) {
        for (var col = 0; col < count; col++) {
            if (qr.isDark(row, col)) {
                var x = Math.round(col * cellWidth);
                var y = Math.round(row * cellHeight);
